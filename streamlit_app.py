...
TRANSLATIONS = {
    "en": {
        ...
        "meters_unit": "meters",
        "history_table_subheader": "📜 History Table (Editable)", # Still used within personal tab
        ...
        "club_performances_tab_title": "📈 Club Performances", # Used as subheader in Club Performances tab
        ...
        "count_col": "Count",
        "training_log_tab_title": "📅 Training Log", # Main tab label for Training
        ...
        "training_log_table_header": "📋 Training Sessions (Editable)", # Subheader for editable training log
        ...
        "feedback_log_tab_label": "💬 Feedback Log", # Main tab label for Feedback
        "feedbacks_overview_tab_label": "💬 Feedbacks", # Sub-tab label
        "edit_feedbacks_sub_tab_label": "📝 Edit Feedbacks", # New sub-tab label
        ...
        "feedback_log_table_header": "📋 Feedback Log (Editable)", # Subheader for editable feedback log
        ...
        "detailed_training_sessions_subheader": "Detailed Training Sessions", # Subheader for training session list
        "training_sessions_sub_tab_label": "🗓️ Training Sessions", # New sub-tab label for Training
        "edit_training_sessions_sub_tab_label": "✏️ Edit Training Sessions", # New sub-tab label for Training
    ...
    "fr": {
        ...
        "feedback_log_tab_label": "💬 Journal des Feedbacks", # Main tab
        "feedbacks_overview_tab_label": "💬 Feedbacks", # Sub-tab (ancienne "Feedbacks")
        "edit_feedbacks_sub_tab_label": "📝 Modifier Feedbacks", # New sub-tab
        ...
        "training_sessions_sub_tab_label": "🗓️ Sessions d'Entraînement", # New sub-tab
        "edit_training_sessions_sub_tab_label": "✏️ Modifier Sessions d'Entraînement", # New sub-tab
    ...
    "nl": {
        ...
        "feedback_log_tab_label": "💬 Feedbacklogboek", # Main tab
        "feedbacks_overview_tab_label": "💬 Feedbacks", # Sub-tab
        "edit_feedbacks_sub_tab_label": "📝 Feedbacks Bewerken", # New sub-tab
        ...
        "training_sessions_sub_tab_label": "🗓️ Trainingssessies", # New sub-tab
        "edit_training_sessions_sub_tab_label": "✏️ Trainingssessies Bewerken", # New sub-tab
...
def main():
    ...
    with st.sidebar.container(border=True):
        ...
        previous_selected_user_key = "current_user_sidebar_selection_prev" # To track change

        if all_known_users_list:
            user_options_sidebar = [_("add_new_user_option", lang)] + all_known_users_list
            
            default_selection_value = st.session_state.get("current_user_sidebar_selection", user_options_sidebar[1] if len(user_options_sidebar) > 1 else user_options_sidebar[0])
            if default_selection_value not in user_options_sidebar:
                default_selection_value = user_options_sidebar[1] if len(user_options_sidebar) > 1 else user_options_sidebar[0]
            ...
            if st.session_state.get(previous_selected_user_key) != selected_option_sidebar:
                st.session_state.privileged_user_authenticated = False
                st.session_state.authenticated_privileged_user = None
                if "password_input_key_for_sidebar" in st.session_state:
                    st.session_state.password_input_key_for_sidebar = "" # Clear password field on user change
                st.session_state[previous_selected_user_key] = selected_option_sidebar 
        ...
        if current_user in PRIVILEGED_USERS:
            if st.session_state.get('authenticated_privileged_user') != current_user:
                st.session_state.privileged_user_authenticated = False
                if "password_input_key_for_sidebar" in st.session_state:
                    st.session_state.password_input_key_for_sidebar = ""

            if not st.session_state.get('privileged_user_authenticated', False):
                st.session_state.password_input_key_for_sidebar = st.text_input(
                    _("enter_access_code_prompt", lang),
                    type="password" 
                )
                
                if st.button(_("unlock_button_label", lang), key="unlock_button_privileges_sidebar"):
                    if st.session_state.password_input_key_for_sidebar == CORRECT_PASSWORD:
    ...
    tab_label_main_feedback_log = _("feedback_log_tab_label", lang) # Main tab for Feedback
    tab_label_club_performances = _("club_performances_overview_tab_label", lang)
    tab_label_divers = _("divers_tab_title", lang)
    tab_label_main_training_log = _("training_log_tab_title", lang) # Main tab for Training
    ...
    if not all_records_loaded and not all_known_users_list and not current_user and not training_log_loaded and not instructor_feedback_loaded:
        ...
        if is_admin_view_authorized:
            tabs_to_display_names_main.append(tab_label_main_feedback_log) # New main feedback tab
            tabs_to_display_names_main.append(tab_label_club_performances)
            tabs_to_display_names_main.extend([tab_label_divers, tab_label_main_training_log, tab_label_performance_log]) # Main training tab
        else:
            # Non-admins only see Personal and Club Performances. "My Feedbacks" is removed.
            tabs_to_display_names_main.append(tab_label_club_performances)
        
        tab_objects_main = st.tabs(tabs_to_display_names_main)

        # Assign tab objects carefully
        tab_personal = tab_objects_main[0]
        current_tab_idx = 1

        # Initialize all possible main tab variables to None
        main_feedback_log_tab_obj = None
        tab_club_performances_view = None
        tab_divers = None
        main_training_log_tab_obj = None
        tab_performance_log = None

        if is_admin_view_authorized:
            main_feedback_log_tab_obj = tab_objects_main[current_tab_idx]; current_tab_idx += 1
            tab_club_performances_view = tab_objects_main[current_tab_idx]; current_tab_idx += 1
            tab_divers = tab_objects_main[current_tab_idx]; current_tab_idx +=1
            ...
            if len(tab_objects_main) > 1: # Check if club performances tab exists for non-admin
                                ...
                                if not history_for_editor_raw:
                                    ...
                                    if st.button(_("save_history_changes_button", lang), key=f"save_hist_{disc_key_sub_tab_user}"):
                                        ...
                                        for index, edited_row_series in edited_df.iterrows():
                                            ...
                                            for i, r_loop in enumerate(temp_all_records_loaded): # Renamed r to r_loop to avoid conflict
                                                if r_loop.get('id') == record_id:
                                                    original_record_index = i
                                                    original_record = r_loop
                                                    break
        ...
        # --- Combined Feedback Log Tab (Admin Only) ---
        if is_admin_view_authorized and main_feedback_log_tab_obj:
            with main_feedback_log_tab_obj:
                # st.subheader(_("feedback_log_tab_label", lang)) # Main tab title
                
                sub_tab_fb_overview_label = _("feedbacks_overview_tab_label", lang) # "Feedbacks"
                sub_tab_fb_edit_label = _("edit_feedbacks_sub_tab_label", lang) # "Edit Feedbacks"
                
                fb_sub_tab1, fb_sub_tab2 = st.tabs([sub_tab_fb_overview_label, sub_tab_fb_edit_label])

                with fb_sub_tab1: # Content from old "Feedbacks Overview" tab
                    st.subheader(_("feedbacks_overview_tab_label", lang)) # Or keep using this as subheader
                    ...
                    with col1_fb_overview:
                        ...
                        selected_diver_filter = st.selectbox(_("filter_by_diver_label", lang), diver_options_filter, key="fb_overview_diver_filter_sub")
                ...
                with fb_sub_tab2: # Content from old "Feedback Log (Editable)" tab
                    st.subheader(_("feedback_log_table_header", lang)) # Or use the sub-tab name "Edit Feedbacks"

                    if not instructor_feedback_loaded:
                        st.info(_("no_feedback_in_log", lang))
                    else:
                        feedback_log_display_list = []
                        for fb_item in sorted(instructor_feedback_loaded, key=lambda x: x['feedback_date'], reverse=True): # Renamed fb to fb_item
                            ...
                            feedback_log_display_list.append({
                                "id": fb_item.get("id"),
                                _("feedback_date_col", lang): fb_item.get("feedback_date"),
                                _("feedback_for_diver_label", lang): fb_item.get("diver_name"),
                                _("instructor_name_label", lang): fb_item.get("instructor_name"),
                                _("training_session_label", lang): training_info,
                                "training_session_id_hidden": fb_item.get("training_session_id"),
                                _("feedback_text_label", lang): fb_item.get("feedback_text"),
                                _("history_delete_col_editor", lang): False
                            })
                        ...
                        edited_feedback_log_df = st.data_editor(
                            ...
                            hide_index=True,
                            key="feedback_log_data_editor_sub", # Changed key to avoid conflict
                            num_rows="dynamic"
                        )

                        if st.button(_("save_feedback_log_changes_button", lang), key="save_feedback_log_changes_sub"): # Changed key
                            feedback_log_changes_made = False
                            temp_feedback_log = [fb_loop.copy() for fb_loop in instructor_feedback_loaded] # Renamed fb to fb_loop
                            ...
                            for edited_row_data in edited_feedback_log_df.to_dict(orient="records"):
                                ...
                                if edited_row_data[_("history_delete_col_editor", lang)] and not is_new_feedback_row:
                                    temp_feedback_log = [fb_loop for fb_loop in temp_feedback_log if fb_loop.get("id") != feedback_id] # Renamed fb to fb_loop
                                ...
                                if is_new_feedback_row:
                                    ...
                                    original_fb_index = next((i for i, fb_item_loop in enumerate(temp_feedback_log) if fb_item_loop.get("id") == feedback_id), -1) # Renamed fb_item to fb_item_loop
                        ...
                        for i_rank_sub_tab, selected_discipline_ranking_key in enumerate(discipline_keys):
                            with ranking_sub_tabs[i_rank_sub_tab]:
                                ...
                                if not sorted_rankings_tab:
                                    ...
                                    st.markdown("---")
                                    st.subheader(_("club_performances_tab_title", lang)) # Subheader for graph section
                        ...
                        for index, edited_row in edited_divers_df.iterrows():
                            ...
                            if name_changed or cert_changed or cert_date_changed or lifras_id_changed or anonymize_changed:
                                ...
                                if name_changed:
                                    ...
                                    old_profile_data['anonymize_results'] = new_anonymize_status 
                                    ...
                                    temp_user_profiles[original_name]['anonymize_results'] = new_anonymize_status
        ...
        # --- Combined Training Log Tab (Admin Only) ---
        if is_admin_view_authorized and main_training_log_tab_obj:
            with main_training_log_tab_obj:
                # st.subheader(_("training_log_tab_title", lang)) # Main tab title

                sub_tab_training_sessions_label = _("training_sessions_sub_tab_label", lang)
                sub_tab_edit_training_sessions_label = _("edit_training_sessions_sub_tab_label", lang)

                training_sub_tab1, training_sub_tab2 = st.tabs([sub_tab_training_sessions_label, sub_tab_edit_training_sessions_label])

                with training_sub_tab1: # "Training Sessions" - list of sessions
                ...
                with training_sub_tab2: # "Edit Training Sessions" - editable table
                    st.subheader(_("training_log_table_header", lang)) # Or "Edit Training Sessions"
                    if not training_log_loaded:
                         st.info(_("no_training_sessions_logged", lang)) # Redundant if first sub-tab shows this
                    else:
                        ...
                        edited_training_df = st.data_editor(
                            ...
                            hide_index=True,
                            key="training_log_editor_sub", # Changed key
                            num_rows="dynamic"
                        )

                        if st.button(_("save_training_log_changes_button", lang), key="save_training_log_changes_sub"): # Changed key
                            ...
                            if is_new_row:
                                ...
                                if original_record_index != -1:
                                    ...
                                    if (original_rec_data.get('date') != edited_date_str or
                                        ...
                                        changes_made_all_perf = True
                                        if edited_user not in user_profiles: # Ensure user profile exists if name changed/added here
                                            user_profiles[edited_user] = {"certification": _("no_certification_option", lang), "certification_date": None, "lifras_id": "", "anonymize_results": False}
